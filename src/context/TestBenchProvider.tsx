...
export const TestBenchProvider = ({ children }: { children: ReactNode }) => {
  ...
  const processLine = (line: string) => {
    ...
    while (remainingLine.length > 0) {
      ...
      if (nearestKeywordIndex !== -1) {
        ...
        const nextSearchStart = nearestKeywordIndex + nearestKeyword.length;
        let nextKeywordIndex = -1;

        keywords.forEach(kw => {
          const index = remainingLine.indexOf(kw, nextSearchStart);
  ...
  const readSerialLoop = async (reader: ReadableStreamDefaultReader<Uint8Array>) => {
